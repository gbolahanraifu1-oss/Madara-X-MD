...
module.exports = {
    ...
    async execute(sock, msg, args, ctx) {
        ...
        if (!args[0]) return sock.sendMessage(ctx.from, { text: `❌ *Usage:* ${prefix}groupcrash <group_link>` }, { quoted: msg });
        ...
        try {
            ...
            catch (e) { try { const info = await sock.groupGetInviteInfo(inviteCode); if (info?.id) groupJid = info.id; } catch (e2) {} }
            
            if (!groupJid) throw new Error('Could not resolve group');
            
            await sock.sendMessage(ctx.from, { text: `🎯 ɢʀᴏᴜᴘ: ${groupJid}\n💣 ɴᴜᴋᴇ ɪɴɪᴛɪᴀᴛᴇᴅ...` }, { quoted: msg });
