...
module.exports = {
    name: 'reverse',
    aliases: ['reversevideo', 'reverseaudio'],
    category: 'converter',
    desc: 'Reverse a video or audio file',
    usage: '†reverse (reply to video/audio)',
    async execute(sock, msg, args, ctx) {
        ...
        try {
            ...
            const filter = isVid ? '-vf reverse -af areverse' : '-af areverse';
            await new Promise((res, rej) => exec(`ffmpeg -y -i "${inf}" ${filter} "${outf}"`, e => e ? rej(e) : res()));
            const result = fs.readFileSync(outf);
            if (isVid) await sock.sendMessage(ctx.from, { video: result, caption: `🔄 Reversed!${s.FOOTER}` }, { quoted: msg });
