...
module.exports = {
    ...
    category: 'media',
    desc: 'TikTok audio only',
    usage: '†tiktokmp3 [url]',
    async execute(sock, msg, args, ctx) {
        const s = ctx.settings;
        const url = args[0];
        if (!url) return ctx.reply(`❌ Provide a TikTok URL.\nUsage: \`${s.prefix}tiktokmp3 [url]\`${s.FOOTER}`);
        await ctx.react('⏳');
        try {
            ...
            if (true) {
                ...
                await sock.sendMessage(ctx.from, { video: buf, caption: `🎬 Downloaded from TikTok${s.FOOTER}` }, { quoted: msg });
            }
