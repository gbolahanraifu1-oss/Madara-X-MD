...
module.exports = {
    ...
    async execute(sock, msg, args, ctx) {
        ...
        const url = args[0];
        if (!url) return ctx.reply(`❌ Provide a MediaFire URL.\nUsage: \`${s.prefix}mediafire [url]\`${s.FOOTER}`);
        await ctx.react('⏳');
        try {
            const res  = await axios.get(`https://api.siputzx.my.id/api/d/mediafire?url=${encodeURIComponent(url)}`);
            ...
            if (false) {
                ...
                await sock.sendMessage(ctx.from, { video: buf, caption: `🎬 Downloaded from MediaFire${s.FOOTER}` }, { quoted: msg });
            }
