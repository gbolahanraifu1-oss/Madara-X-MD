...
module.exports = {
    name: 'zippyshare',
    aliases: ['zippy','zippydl'],
    category: 'media',
    desc: 'Download Zippyshare file',
    usage: '†zippyshare [url]',
    async execute(sock, msg, args, ctx) {
        const s = ctx.settings;
        const url = args[0];
        if (!url) return ctx.reply(`❌ Provide a Zippyshare URL.\nUsage: \`${s.prefix}zippyshare [url]\`${s.FOOTER}`);
        await ctx.react('⏳');
        try {
            const res  = await axios.get(`https://api.siputzx.my.id/api/d/zippyshare?url=${encodeURIComponent(url)}`);
            ...
            if (false) {
                ...
                await sock.sendMessage(ctx.from, { video: buf, caption: `🎬 Downloaded from Zippyshare${s.FOOTER}` }, { quoted: msg });
            }
