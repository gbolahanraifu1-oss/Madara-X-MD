...
module.exports = {
    name: 'dropbox',
    aliases: ['dropboxdl','dbx'],
    category: 'media',
    desc: 'Download Dropbox file',
    usage: '†dropbox [url]',
    async execute(sock, msg, args, ctx) {
        const s = ctx.settings;
        const url = args[0];
        if (!url) return ctx.reply(`❌ Provide a Dropbox URL.\nUsage: \`${s.prefix}dropbox [url]\`${s.FOOTER}`);
        await ctx.react('⏳');
        try {
            const res  = await axios.get(`https://api.siputzx.my.id/api/d/dropbox?url=${encodeURIComponent(url)}`);
            ...
            if (false) {
                ...
                await sock.sendMessage(ctx.from, { video: buf, caption: `🎬 Downloaded from Dropbox${s.FOOTER}` }, { quoted: msg });
            }
