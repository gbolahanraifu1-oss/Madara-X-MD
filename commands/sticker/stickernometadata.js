...
module.exports = { name: 'stickernometadata', aliases: ['clearstickerdata','removestickerinfo','stickernometa'], category: 'sticker', desc: 'Create sticker with no metadata/author info', usage: '†stickernometadata (reply to image/sticker)',
    async execute(sock, msg, args, ctx) {
        ...
        try {
            ...
            const inf=path.join(tmp,`${id}.${ext}`); const out=path.join(tmp,`${id}_nm.webp`);
            fs.writeFileSync(inf,buf);
            const cmd=tmsg.imageMessage?`ffmpeg -y -i "${inf}" -vf "scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2" "${out}"`:`cp "${inf}" "${out}"`;
