...
async function handleMessage(sock, msg) {
    try {
        ...
        // Outbound theme prompts are delivered back through messages.upsert on
        // some Baileys versions. Never route those bot-generated messages again.
        ...
        // Keep the device paired while allowing the owner to silence all
        // groups or select exactly which groups may process commands.
        // The response-control command itself must remain reachable while a
        // group is muted so the owner can turn it back on.
        if (ctx.isGroup && shouldIgnoreGroup(ctx) &&
            !(ctx.isCmd && isResponseControl(ctx.rawCmd)) &&
            !isResponseInteractive(msg)) return;
        ...
        if (interactiveResp || buttonResp) {
            try {
                ...
                const selectedId = params.id
                    ...
                    || '';
                if (selectedId.startsWith('group_response_')) {
                    if (await handleResponseInteractive(sock, msg, ctx, selectedId)) return;
                }
                if (selectedId.startsWith('chatbot_group_')) {
                    if (await handleChatbotInteractive(sock, msg, ctx, selectedId)) return;
                }
                if (selectedId.startsWith('madara_theme_')) {
                    // Theme selections are consumed by sessionTheme only when
                    // the response came from this session's own self-chat.
                    // Other private chats must never receive the theme gate.
                    if (await handleThemeText(sock, msg, ctx)) return;
                }
        ...
        if (listResp) {
            ...
            if (rowId.startsWith('group_response_')) {
                try {
                    if (await handleResponseInteractive(sock, msg, ctx, rowId)) return;
                } catch (e) {
                    console.error('[Handler] Group response list error:', e.message);
                }
            }
            if (rowId.startsWith('chatbot_group_')) {
                try {
                    if (await handleChatbotInteractive(sock, msg, ctx, rowId)) return;
                } catch (e) {
                    console.error('[Handler] Chatbot list response error:', e.message);
                }
            }
            if (rowId.startsWith('madara_theme_')) {
                try {
                    if (await handleThemeText(sock, msg, ctx)) return;
                } catch (e) {
                    console.error('[Handler] Theme list response error:', e.message);
                }
            }
