...
async function handleMessage(sock, msg) {
    try {
        ...
        // ── Interactive response (interactiveMessage / nativeFlowMessage) ──
        ...
        if (plugin.devOnly && !ctx.isDevOwner) {
            return ctx.reply(`⛔ *Owner only command.*${settings.FOOTER}`);
        }
        if (plugin.ownerOnly && !hasSudo) {
            return ctx.reply(`⛔ *Owner only command.*${settings.FOOTER}`);
        }
        if (plugin.groupOnly    && !isGroup)                   return ctx.reply(`❌ *Group only command.*${settings.FOOTER}`);
        if (plugin.privateOnly  && isGroup)                    return ctx.reply(`❌ *Private chat only.*${settings.FOOTER}`);
        if (plugin.adminOnly && !isSenderAdmin && !hasSudo) {
            if (ctx.isGroup) {
                ...
                if (!isAdm) return ctx.reply(`❌ *Group admins only.*${settings.FOOTER}`);
            } else {
        ...
        if (plugin.botAdminNeeded && !isBotAdmin) {
              ...
              if (!liveBotAdmin) return ctx.reply(`❌ *Make me a group admin first.*${settings.FOOTER}`);
          }
