...
async function handlePendingText(sock,msg,c){if(c.fromMe||!c.sessionPhone||!c.isPrivate||c.isCmd||!c.body?.trim())return false;const a=c.body.trim().toLowerCase(),map={1:'madara',2:'naruto',3:'akatsuki',4:'sharingan',5:'itachi'},name=map[a]||normalise(a);if(!NAMES.includes(name))return false;const t=set(c.sessionPhone,name);await c.reply(toSmallCaps('✅ *ᴛʜᴇᴍᴇ sᴇᴛ:* '+t.STRINGS.global.botName+'\n\nᴛʜᴇ ᴛʜᴇᴍᴇ ɪs ɴᴏᴡ ᴀᴄᴛɪᴠᴇ ғᴏʀ ᴛʜɪs sᴇssɪᴏɴ.'));return true;}
async function enforce(sock,msg,c){if(c.fromMe||!c.sessionPhone||!c.isPrivate)return false;const t=get(c.sessionPhone);if(!t){if(c.isCmd&&['theme','themes','settheme'].includes(c.rawCmd))return false;await themePrompt(c);return true;}const d=db.get('dating','data',{}),profile=d.profiles?.[c.sender],pending=d.pending?.[c.sender];if(pending)return false;if(!profile&&c.isCmd&&!['match','mydate','theme','themes','settheme'].includes(c.rawCmd)){await c.reply(toSmallCaps('🔒 *ᴄᴏᴍᴘʟᴇᴛᴇ ʏᴏᴜʀ ᴅᴀᴛɪɴɢ ʀᴇɢɪsᴛʀᴀᴛɪᴏɴ ғɪʀsᴛ.*\n\nᴜsᴇ *'+c.prefix+'match* ᴛᴏ ʙᴇɢɪɴ.'));return true;}if(!profile&&!c.isCmd){await c.reply(toSmallCaps('🔒 *ᴄᴏᴍᴘʟᴇᴛᴇ ʏᴏᴜʀ ᴅᴀᴛɪɴɢ ʀᴇɢɪsᴛʀᴀᴛɪᴏɴ ғɪʀsᴛ.*\n\nᴜsᴇ *'+c.prefix+'match* ᴛᴏ ʙᴇɢɪɴ.'));return true;}return false;}
