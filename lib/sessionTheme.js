...
async function themePrompt(c, options = {}) {
    ...
    // Do not emit the same first-run prompt again while this session is
    // already waiting for the owner's selection.
    if (options.markPending !== false && !markedPending) return null;
    try {
...
async function handlePendingText(sock, msg, c) {
    if (!c?.isPrivate || !c.sessionPhone || c.isCmd || !isPending(c.sessionPhone)) return false;
...
async function enforce(sock, msg, c) {
    if (!c?.isPrivate || !c.sessionPhone) return false;
