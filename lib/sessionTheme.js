...
async function themePrompt(c) {
    // Interactive picker for clients that support native-flow messages.
    // Plain numbered text remains the fallback for older WhatsApp clients.
    if (c?.sock?.relayMessage && c.from) {
        try {
            const { sendInteractiveList } = require('./baileysHelper');
            return await sendInteractiveList(c.sock, c.from, {
                body: toSmallCaps(promptText()),
                footer: toSmallCaps('ᴘᴀɪʀᴇᴅ sᴇssɪᴏɴ ᴛʜᴇᴍᴇ'),
                btnTitle: '🎨 ᴄʜᴏᴏsᴇ ᴛʜᴇᴍᴇ',
                sections: [{
                    title: 'ᴍᴀᴅᴀʀᴀ ᴛʜᴇᴍᴇs',
                    rows: NAMES.map((name, i) => ({
                        title: `${i + 1}. ${name}`,
                        description: DESCRIPTIONS[name],
                        rowId: `madara_theme_${name}`,
                    })),
                }],
            }, c.msg || null);
        } catch {}
