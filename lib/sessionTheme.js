...
async function handlePendingText(sock,msg,c){if(!c.sessionPhone||!c.isPrivate||c.isCmd||!c.body?.trim())return false;const a=c.body.trim().toLowerCase(),map={1:'madara',2:'naruto',3:'akatsuki',4:'sharingan',5:'itachi'},name=map[a]||normalise(a);if(!NAMES.includes(name))return false;const t=set(c.sessionPhone,name);await c.reply(toSmallCaps('✅ *ᴛʜᴇᴍᴇ sᴇᴛ:* '+t.STRINGS.global.botName+'\n\nᴛʜᴇ ᴛʜᴇᴍᴇ ɪs ɴᴏᴡ ᴀᴄᴛɪᴠᴇ ғᴏʀ ᴛʜɪs sᴇssɪᴏɴ.'));return true;}
