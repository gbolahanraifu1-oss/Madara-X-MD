...
// Telegram supports 3 button colors: primary (blue), success (green), danger (red)
// These cycle automatically to create the "auto-change" effect
...
function getStyleForIndex(idx) {
    return BUTTON_STYLES[idx % BUTTON_STYLES.length];
}
...
function startTelegramBot(startSession, activeSessions) {
    ...
    const kb = (...rows) => ({
        inline_keyboard: rows.map(row =>
            row.map(btn => {
                // If button already has a style, keep it. Otherwise auto-assign cycling style.
    ...
    function userMenuKeyboard() {
        return kb(
            ...
            [
                { text: '🏓 Ping', callback_data: 'btn_ping', style: getNextStyle() },
                { text: '⏰ Runtime', callback_data: 'btn_runtime', style: getNextStyle() }
            ],
            [
                { text: '📊 Stats', callback_data: 'btn_stats', style: getNextStyle() },
                { text: '📧 Report', callback_data: 'btn_report', style: getNextStyle() }
            ],
            [
                { text: '🎬 Tutorial', callback_data: 'btn_tutorial', style: getNextStyle() },
                { text: '❓ Help', callback_data: 'btn_help', style: getNextStyle() }
            ],
            [
    ...
    function adminMenuKeyboard() {
        return kb(
            [
                { text: '👥 Users', callback_data: 'btn_users', style: getNextStyle() },
                { text: '📱 Sessions', callback_data: 'btn_listpair', style: getNextStyle() }
            ],
            [
            ...
            [
                { text: '🔍 Check', callback_data: 'btn_checkuser', style: getNextStyle() },
                { text: '🔧 Mode', callback_data: 'btn_maintenance', style: getNextStyle() }
            ],
            [
                { text: '📋 Logs', callback_data: 'btn_logs', style: getNextStyle() },
    ...
    // ── Auto-cycle button colors every 5 seconds ─────────────────────────
    const colorCycleInterval = setInterval(async () => {
        // Cycle through the 3 styles
        styleIndex = (styleIndex + 1) % BUTTON_STYLES.length;
        ...
        for (const [chatId, sess] of tgSessions.entries()) {
            ...
            try {
                // Regenerate with new style assignments
                ...
                // Silent — message might be too old or deleted
            }
        }
    }, 5000);
    ...
    async function dispatchMessage(chatId, m) {
        ...
        if (sess.state === 'delsession_id') {
            ...
            tgSessions.set(chatId, { ...sess, state: null });
            if (!phone) return await render(chatId, box('Delete Session', [`❌ Invalid number — try again.`]), { reply_markup: kb([{ text: '🔙 Back', callback_data: 'btn_adminmenu', style: 'primary' }]) });
            const existed = await clearSession(phone).catch(() => false);
            return await render(chatId,
                box('Delete Session', [existed ? `✅ Session *+${phone}* deleted and disconnected.` : `⚠️ No session found for *+${phone}*.`]),
                { reply_markup: kb([{ text: '❌ Delete Another', callback_data: 'btn_delsession', style: 'danger' }], [{ text: '🏠 Main Menu', callback_data: 'btn_mainmenu', style: 'primary' }]) }
            );
        }
    ...
    console.log('✅ Telegram pairing bot started with auto-cycling 3-color buttons');
}
