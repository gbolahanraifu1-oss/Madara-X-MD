...
// ║   3-Color Auto-Cycling (Full Re-render)            ║
// ╚══════════════════════════════════════════════════════╝
...
function startTelegramBot(startSession, activeSessions) {
    ...
    function cycleStyle(offset = 0) {
        return BUTTON_STYLES[(styleIndex + offset) % BUTTON_STYLES.length];
    }

    function mainMenuKeyboard() {
        return kb(
            [{ text: '📱 Pair WhatsApp', callback_data: 'btn_pair', style: cycleStyle(0) }],
            [
                { text: '👤 User Menu', callback_data: 'btn_usermenu', style: cycleStyle(1) },
                { text: '👮 Admin Menu', callback_data: 'btn_adminmenu', style: cycleStyle(2) }
            ],
            [
                { text: '🔒 Owner Menu', callback_data: 'btn_ownermenu', style: cycleStyle(0) }
            ],
            ...linkRows().map((row, index) => row.map(btn => ({ ...btn, style: cycleStyle(index + 1) })))
        );
    }
    ...
    // ── Auto-cycle button colors — FULL RE-RENDER every 5s ───────────────
    const colorCycleInterval = setInterval(async () => {
        ...
        for (const [chatId, sess] of tgSessions.entries()) {
            ...
            try {
                const freshText = mainMenuText();
                const freshMarkup = mainMenuKeyboard();
                
                await bot.editMessageText(freshText, {
                    chat_id: chatId,
                    message_id: sess.lastMsgId,
                    parse_mode: 'Markdown',
                    reply_markup: sanitizeMarkup(freshMarkup),
                });
            } catch (e) {
                // Silent — message might be too old or "not modified"
            }
        }
    }, 5000);
