...
function requestCodeOnQr(sock, phone) {
    return new Promise((resolve, reject) => {
        ...
        const handler = (update) => {
            ...
            if (qr) {
                sock.requestPairingCode(phone, settings.pairingCode)
...
// Telegram has no CSS animation for inline keyboards. Rotate the supported
// style with one bounded timer per chat, and only while a menu is visible.
const autoColorTimers = new Map();
function stopAutoColor(chatId) {
    const timer = autoColorTimers.get(chatId);
    if (timer) clearInterval(timer);
    autoColorTimers.delete(chatId);
}
function startAutoColor(chatId) {
    stopAutoColor(chatId);
    const timer = setInterval(async () => {
        const sess = tgSessions.get(chatId) || {};
        if (!sess.lastMsgId || !sess.menuMarkup) return stopAutoColor(chatId);
        const markup = applyAutoColor(chatId, sess.menuMarkup);
        try {
            await bot.editMessageReplyMarkup(markup, { chat_id: chatId, message_id: sess.lastMsgId });
        } catch (e) {
            const message = String(e.message || '').toLowerCase();
            if (!message.includes('not modified')) {
                console.error('[Telegram] auto-color error:', e.message);
                stopAutoColor(chatId);
            }
        }
    }, 5_000);
    if (timer.unref) timer.unref();
    autoColorTimers.set(chatId, timer);
}
...
async function render(chatId, text, opts = {}) {
    const sess = tgSessions.get(chatId) || {};
    const { reply_markup, ...rest } = opts;
    const baseMarkup = reply_markup ? sanitizeMarkup(reply_markup) : null;
    const payload = {
        parse_mode: 'Markdown',
        ...rest,
        ...(baseMarkup ? { reply_markup: applyAutoColor(chatId, baseMarkup) } : {}),
    };
    if (baseMarkup) {
        tgSessions.set(chatId, { ...(tgSessions.get(chatId) || sess), menuMarkup: baseMarkup });
    } else {
        stopAutoColor(chatId);
        const current = tgSessions.get(chatId) || sess;
        const { menuMarkup, ...withoutMenu } = current;
        tgSessions.set(chatId, withoutMenu);
    }
        ...
        if (sess.lastMsgId) {
            try {
                ...
                if (e.message?.includes('not modified')) {
                    if (baseMarkup) startAutoColor(chatId);
                    return;
                }
            }
        }
        ...
        try {
            ...
            const current = tgSessions.get(chatId) || sess;
            tgSessions.set(chatId, { ...current, lastMsgId: sent.message_id });
            if (baseMarkup) startAutoColor(chatId);
        } catch (e) {
    ...
    async function executePairing(dmChatId, phone, groupId = null) {
        try {
            ...
            // An already-connected socket will not emit a new QR event.
            if (sock?.user?.id || activeSessions.get(phone)?.connected === true) {
                throw new Error('This WhatsApp number is already paired. Unpair it first, then try again.');
            }
