...
const { askProviderWithFallback } = require('../commands/ai/ai');

async function aiQuery(prompt, options = {}) {
    const provider = ['openai', 'grok', 'claude'].includes(options.provider)
        ? options.provider
        : (process.env.DEFAULT_AI_PROVIDER || 'grok');
    return askProviderWithFallback(provider, prompt, options.tone || 'warm');
}
