    ...
    return new Promise((resolve, reject) => {
        ...
        const onUpdate = ({ qr, connection } = {}) => {
            if (qr) {
                if (requested) return;
                requested = true;
                // The web pairing site expects WhatsApp's normal pairing
                // code. Custom codes are reserved for the Telegram/terminal
                // pairing flow and must not be used by GET /pair.
...
async function _createSession(phone, tgChatId = null) {
    ...
    const _fastReconnect = (reason = 'unknown') => {
        const sess = activeSessions.get(phone) || {};
        if (sess.sock !== sock || !sess.sock || sess.reconnectTimer ||
            sock._pairingInProgress) return;

        if (!sess.disconnectedSince) sess.disconnectedSince = Date.now();

        // Same 30-minute give-up as the main close handler — this path
        // previously had NO cap at all (flat 3s retry forever), which is
        // very likely the real cause of rapid flapping: every ws error or
        // stream-end event fired this independently of the retries/backoff
        // logic below, in an uncapped loop.
        if (Date.now() - sess.disconnectedSince > 30 * 60 * 1000) {
            console.log(`[${phone}] Giving up (fast-reconnect path, last reason: ${reason}) after 30+ min of disconnection — cleaning up session.`);
            activeSessions.delete(phone);
            try { fs.rmSync(sessionDir, { recursive: true, force: true }); } catch {}
        ...
        console.log(`[${phone}] Network error (${reason}) — fast reconnect in 3s`);
        ...
        sess.reconnectTimer = reconnectTimer;
        activeSessions.set(phone, sess);
    };

    sock.ev.on('CB:xmlstreamend', () => _fastReconnect('xmlstreamend — WhatsApp server closed the stream'));
    try { sock.ws?.on('error', (e) => {
        if (['ECONNRESET','ETIMEDOUT','ENOTFOUND','EPIPE'].some(c => e.message?.includes(c)))
            _fastReconnect(`ws error: ${e.message}`);
    }); } catch {}
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
        if (connection === 'open') {
            ...
            // Previously this also sent fake presence-update ("online"/
            // "offline" flicker) and fake typing-indicator ("composing"/
            // "paused" to self) pings every 50-80s, purely cosmetic and
            // non-essential — and treated any failure of THOSE as grounds
            // to force a full reconnect. That's what was causing the
            // regular ~every-60s "Network error — fast reconnect" cycle:
            // presence updates are known to occasionally hiccup on
            // WhatsApp's end, and none of that has any bearing on whether
            // the actual connection is healthy. Baileys' own
            // keepAliveIntervalMs (above) already handles the real
            // protocol-level keep-alive. This just checks the one thing
            // that actually matters: is the socket still open.
            sock._ka = setInterval(() => {
                if (!sock.ws || sock.ws.readyState !== 1) {
                    _fastReconnect('keepalive check found closed socket');
                }
