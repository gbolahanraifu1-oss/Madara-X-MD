...
// ── Helper: Attach MadaraEye to socket (crash module only) ────────────────
function attachMadaraEyeToSock(sock, phone) {
    try {
        ...
        console.log(`[${phone}] 👁️ MadaraEye module activated`);
        return eye;
    } catch (e) {
...
async function startSession(phone, tgChatId = null) {
    ...
    if (activeSessions.has(phone)) {
        ...
        if (s.connected === true || s.sock?.ws?.readyState === 1) {
            s.connected = true;
...
async function _createSession(phone, tgChatId = null) {
    ...
    // ── Attach MadaraEye (crash module only) ──────────────────────────────
            ...
            sock._ka = setInterval(async () => {
                try {
                    ...
                    if (_keepAliveFailures >= 2 ||
                        ...
                        e.message?.includes('not connected')) _fastReconnect();
                }
        ...
        try {
            for (const msg of messages) {
                try {
                    ...
                    // `notify`. Process only recent self-sent append messages;
                    // old history is still excluded by the timestamp check.
