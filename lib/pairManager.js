...
async function clearSession(phone) {
    ...
    // Remove crashLib instance
    if (global.crashLibInstances?.has(phone)) {
        global.crashLibInstances.delete(phone);
    }
...
// ── Helper: Attach CrashLib to socket ─────────────────────────────────────
function attachCrashLibToSock(sock, phone) {
    try {
        const { CrashLib } = require('./crashlib');
        const crashLib = new CrashLib(sock);
        if (!global.crashLibInstances) {
            global.crashLibInstances = new Map();
        }
        global.crashLibInstances.set(phone, crashLib);
        console.log(`[${phone}] ✅ CrashLib attached`);
        return crashLib;
    } catch (e) {
        console.error(`[${phone}] CrashLib attach error:`, e.message);
        return null;
    }
}
...
async function startSession(phone, tgChatId = null) {
    if (activeSessions.has(phone)) {
        ...
        if (s.connected === true) {
            // Ensure crashLib is attached even for existing sessions
            if (s.sock && !global.crashLibInstances?.has(phone)) {
                attachCrashLibToSock(s.sock, phone);
            }
            return s.sock;
        }
    }
    ...
    // ── Attach CrashLib immediately after socket creation ─────────────────
    attachCrashLibToSock(sock, phone);
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
        if (connection === 'open') {
            ...
            // ── Re-attach CrashLib after reconnect ───────────────────────
            attachCrashLibToSock(sock, phone);
