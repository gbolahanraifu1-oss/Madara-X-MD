...
async function buildCtx(sock, msg) {
    ...
    // Right after a reconnect there's a real window where a message event
    // can fire before sock.user is populated yet. Previously this crashed
    // on sock.user.id with no guard at all — a frequent, recurring error
    // in the logs, especially during reconnect-heavy periods. Nothing
    // reliable can be built without a real sender identity here, so skip
    // this one message rather than crash or misattribute the sender.
    if (fromMe && !sock.user?.id) {
        const err = new Error('Transient: sock.user not yet populated (post-reconnect race)');
        err.transient = true;
        throw err;
    }
    ...
    if (isGroup) {
        try {
            ...
            if (cached && (now - cachedTs) < TTL) {
                ...
                try {
                    groupMeta = await withTimeout(
                        sock.groupMetadata(from),
                        GROUP_METADATA_TIMEOUT_MS,
                        null,
                    );
                    if (!groupMeta) throw new Error('Group metadata request timed out');
    ...
    // A session's theme belongs only to its own self-chat.  `fromMe` alone
    // is not enough: the bot also sets it on messages it sends to strangers,
    // which used to make the theme gate leak into unrelated private chats.
    const isSessionOwnerChat = isPrivate && (
        fromMe && (
            normNum(from) === pairedPhone ||
            normNum(from) === sessionNum ||
            (sessionLid && normNum(from) === sessionLid)
        )
        || normNum(from) === pairedPhone
        || normNum(from) === sessionNum
        || (sessionLid && normNum(from) === sessionLid)
    ...
    const sessionSettings = new Proxy(settings, {
        get(target, prop) {
            ...
            if (prop === 'botName') return sessionTheme.string(sessionPhone, 'botName', target.botName);
            if (prop === 'botBrand') return sessionTheme.string(sessionPhone, 'botName', target.botBrand);
            return target[prop];
        },
